...
import { LayoutDashboard, Receipt, CreditCard, BarChart3, TrendingUp, Car, Wallet, Building, ArrowLeft, Download, Upload, MoreVertical } from "lucide-react";
...
export function BottomNav() {
  ...
  const renderRoot = () => <div className="flex items-center justify-between gap-1 px-2 py-1">
      <div className="flex flex-1 items-center justify-between gap-2">
        {ROOT_ITEMS.map(item => {
        const Icon = item.icon;
        const hasActiveChild = NAV_SECTIONS[item.id].items.some(sub => isPathActive(sub.to));
        return <button key={item.id} type="button" onClick={() => handleRootClick(item.id)} className={cn("flex flex-1 flex-col items-center justify-center gap-0.5 rounded-full px-2 py-1 text-[11px] font-medium transition-colors", hasActiveChild ? "text-primary" : "text-muted-foreground hover:text-foreground")}>
              <div className={cn("flex h-8 w-8 items-center justify-center rounded-full text-current transition-colors", hasActiveChild && "bg-primary/10")}>
      ...
      <div className="flex items-center justify-end px-1">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors" aria-label="Mais opções">
  ...
  const renderSection = (sectionId: SectionId) => {
    ...
    return <div className="flex flex-col gap-1 px-2 py-1">
        <div className="flex items-center gap-2 px-1 pb-1">
          <button type="button" onClick={() => setLevel("root")} className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-medium text-muted-foreground hover:text-foreground hover:bg-muted/60">
  ...
  return <nav className="fixed bottom-4 inset-x-0 z-40 flex justify-center md:hidden pointer-events-none">
